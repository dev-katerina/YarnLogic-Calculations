...
@pytest.mark.parametrize("loops,rows,height,width,expected_loop,expected_row", [
    (6, 6, 1, 1, 6, 6),
    (10, 5, 2, 4, 2.5, 2.5),
    (8, 4, 2.5, 4.0, 2.0, 1.6),
])
...
@pytest.mark.parametrize("loops,rows,height,width", [
    (-1, 6, 1, 1),
    (6, -1, 1, 1),
    (6, 6, -1, 1),
    (6, 6, 1, -1),
...
@pytest.mark.parametrize("orig_loops,orig_rows,target_loops,target_rows,expected_loops_k,expected_rows_k", [
    (5, 5, 10, 10, 2.0, 2.0),
    (10, 10, 5, 5, 0.5, 0.5),
    (2, 4, 6, 8, 3.0, 2.0),
    (8, 6, 4, 3, 0.5, 0.5),
...
def test_calculate_density_conversion(orig_loops, orig_rows, target_loops, target_rows, expected_loops_k, expected_rows_k):
...
@pytest.mark.parametrize("orig_loops,orig_rows,target_loops,target_rows", [
...
def test_calculate_density_conversion_invalid_parameters(orig_loops, orig_rows, target_loops, target_rows):
    original = Density(loops=orig_loops, rows=orig_rows)
    target = Density(loops=target_loops, rows=target_rows)
